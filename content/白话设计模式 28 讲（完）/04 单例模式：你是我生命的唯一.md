...
![img](assets\b6726f60-c390-11e8-b5ca-0da8fdb41124.jpg)
...
在 Python 3 的类中，* 负责对象的创建，而 * 负责对象的初始化；* 是一个类方法，而 * 是一个对象方法。

* 是我们通过类名进行实例化对象时自动调用的，* 是在每一次实例化对象之后调用的，* 方法创建一个实例之后返回这个实例对象，并将其传递给 * 方法的 self 参数。

在上面的示例代码中，我们定义了一个静态的 * 类变量，用来存放 Singleton1 的对象，* 方法每次返回同一个*（若未初始化，则进行初始化）。因为每一次通过 s = Singleton1() 的方式创建对象时，都会自动调用 * 方法来初始化实例对象；因此 * 的作用就是确保只对 * 对象进行一次初始化，故事剧情中的代码就是用这种方式实现的单例。
...
Python 中 * 和 * 都是 public 的，所以我们需要通过重写 * 和 * 方法来改造对象的创建过来，从而实现单例模式。如果你要更详细地了解 Python 中 * 和 * 的原理和用法，请参见《》。
